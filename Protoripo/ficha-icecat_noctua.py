...
import json # Puedes remover esta importación si no la usas en otro lado
import requests
import pandas as pd
...
def actualizar_fichas_icecat():
    ...
    try:
        ...
        query = "SELECT csku,cmarca FROM tbl_produdcto_icecat;"
    ...
    def hacer_llamada(idx, product_code, brand):
        ...
        try:
            ...
            if response.status_code == 200:
                # -----------------------------------------------------
                # MODIFICACIÓN: Tomamos el texto plano (str) directamente 
                # en lugar de hacer parse con response.json()
                # -----------------------------------------------------
                raw_data = response.text 
                print(f"✅ {idx} - {product_code} ({brand}) OK")
                contador_exitos += 1
                return {"index": idx, "sku": product_code, "brand": brand, "data": raw_data}
            else:
    ...
    # ==========================
    # MODIFICACIÓN: Al no usar json.dumps, usamos el string directamente de r["data"]
    df_resultados = pd.DataFrame([
        {
            "sku": r["sku"],
            "marca": r["brand"],
            "jsonrespuesta": (
                r["data"]  # Aquí ya es un string (response.text)
                .replace('\x00', '')
                .replace('\\u0000', '')
            )
        } for r in resultados
    ...
    cursor = conn.cursor()
    query = """UPDATE tbl_produdcto_icecat SET json_icecat = %s WHERE csku = %s;"""
    errores_sql = []
